  function data = poisson2_cerr(xm, varargin)
%|function data = poisson2_cerr(xm, [options])
%|
%| in
%|	'xm'		float		
%|
...
if streq_cerr(xm, 'test'), poisson2_test_cerr, return, end
...
data(xm > 0) = poisson2_positive_cerr(col_cerr(xm(xm > 0)), arg.factor);

%
% poisson2_positive()
%
function data = poisson2_positive_cerr(xm, factor)
sx = sqrt(2.0 * xm);
lx = log(xm);
