function OSL = oslPSFSimData(nFWPTtotal,RS,CTAC,initPT,PSF, ...
    wcc,vox,iterNUM,subNUM,lambda)
%"mlemPSFSimData"
%   Reconstructs PET-like images via OSEM w/ psf
%
% CRS, 08/01/2013
%
%Usage:
%   [OSLpsf] = mlemPSFSimData(nFWPTtotal,RS,CTAC,PSF,POST,wcc,radBin,tanBin,simSize,zSlice,iterNUM,reconType)
...
%
%% Reconstructs OSL images w/ psf
epsilon = 1E-8;
dX = [  0  0  0 ;  -1  2 -1 ;  0  0  0];
dY = dX';
...
OSL     = zeros([vox.petOut.nxn iterNUM]);
...
for i = 1:vox.petOut.rtz(3)
    ...
    for j = 1:iterNUM
        for k = 1:subNUM
            ...
            delX = imfilter(f,dX,'replicate','same','conv');
            delY = imfilter(f,dY,'replicate','same','conv');
            TV1 = lambda * (abs(delX) + abs(delY) + epsilon);
            
            f = disk.*f.*hTg ./ ( subH1(:,:,k,i) + TV1 );
