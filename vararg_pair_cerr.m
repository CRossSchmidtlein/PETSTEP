 function [opt, extra] = vararg_pair_cerr(opt, varargs, varargin)
%function [opt, extra] = vararg_pair_cerr(opt, varargs, [options])
...
if nargin == 1 && streq(opt, 'test'), vararg_pair_test_cerr, clear, return, end

base = [caller_name_cerr ' arg: ']; % for printing
...
local.subs = {};
local.allow_new = 0;
local = vararg_pair_do_cerr(local, varargin, {}, 0, 0);

allow_extra = nargout > 1;
[opt extra] = vararg_pair_do_cerr(opt, varargs, local.subs, local.allow_new, ...
	allow_extra, base);
...
%
function [opt, extra] = ...
	vararg_pair_do_cerr(opt, varargs, subs, allow_new, allow_extra, base)
...
for ii=1:npair
	...
	if ~isempty(subs)
		arg = vararg_pair_sub_cerr(arg, subs1, subs2);
	end

	[opt ex] = vararg_pair_put_cerr(opt, arg, val, allow_new, allow_extra);
	if allow_extra && ~isempty(ex)
		extra = [extra(:); ex(:)]'; % faster than {extra{:}, ex{:}};
	end

	if isfield(opt, 'chat') && opt.chat && isempty(ex)
		show_pair_cerr(arg, val, base)
	end
end
...
%
function arg = vararg_pair_sub_cerr(arg, subs1, subs2)
...
%
function [opt, extra] = vararg_pair_put_cerr(opt, arg, value, allow_new, allow_extra)
...
%
function vararg_pair_check_cerr(opt)
...
% show_pair()
%
function show_pair_cerr(name, value, base)
...
% test routine
%
function vararg_pair_test_cerr
...
opt = vararg_pair_cerr(opt, args, 'subs', sub);
jf_equal(opt.a, 1)
jf_equal(opt.b, 2)
jf_equal(opt.c.d, 3)
jf_equal(opt.c.e, 0)
...
opt = vararg_pair_cerr(opt, {'z', 'new'}, 'allow_new', 1);
if ~isequal(opt.z, 'new'), fail('bug'), end
try % typo in name should yield error
	opt = vararg_pair_cerr(opt, {'verify_bad_test!', 'new'}, 'allow_new', 0)
catch
	return
end
